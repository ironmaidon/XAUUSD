...
The exchange adapter is an anti-corruption boundary. Strategy and risk code depend on normalized providers, never HTTP/WebSocket details.
...
Transport objects own connection lifecycles. Public and private WebSockets reconnect with bounded exponential backoff and restore subscriptions. Execution safety, reconciliation, heartbeat health, persistence, and mode gates remain separate services.
...
M12 adds a live limit-order provider behind `LiveTradingGuard`. The provider has no market-order method. Construction never arms it. An order is permitted only after the static environment gate, LIVE mode, credentials, current successful reconciliation, healthy dead-man heartbeat, inactive kill switch, and an exact manual confirmation all pass. Any dynamic-gate failure immediately disarms the process.
